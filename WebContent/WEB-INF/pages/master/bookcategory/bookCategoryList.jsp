    	...
    	<td height="100%" valign="top">
			<table cellspacing="0" cellpadding="0" id="tableList" width="100%">
	        	<thead>
	            	<tr>
	            		<td class="hidden"></td>
	            		<td class="listHeader">Kode Kategori</td> 
	            		<td class="listHeader">Deskripsi</td> 
	             	</tr>
	        	</thead>
				<tbody></tbody>	
         	</table>
...
$(document).ready(function(){
	showList(1);
	$('#'+'<%=SystemConstant.MenuCode.BOOK_CATEGORY%>').addClass('tabActive');
});
...
function populateTable(data){
	...
	$.each(data,function(i){
		tBody += 
   			'<tr>'+
   				'<td class="hidden">'+this.pkBookCategory+'</td>'+
   				'<td class="listItem">'+this.categoryCode+'</td>'+
