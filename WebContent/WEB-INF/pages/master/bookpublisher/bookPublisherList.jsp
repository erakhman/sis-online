    	...
    	<td height="100%" valign="top">
			<table cellspacing="0" cellpadding="0" id="tableList" width="100%">
	        	<thead>
	            	<tr>
	            		<td class="hidden"></td>
	            		<td class="listHeader">Kode</td> 
	            		<td class="listHeader">Nama</td> 
	            		<td class="listHeader">Alamat</td> 
	            		<td class="listHeader">No. Telp</td> 
	             	</tr>
	        	</thead>
				<tbody></tbody>	
         	</table>
...
$(document).ready(function(){
	showList(1);
	$('#'+'<%=SystemConstant.MenuCode.BOOK_PUBLISHER%>').addClass('tabActive');
});
...
function populateTable(data){
	...
	$.each(data,function(i){
		tBody += 
   			'<tr>'+
   				'<td class="hidden">'+this.pkBookPublisher+'</td>'+
   				'<td class="listItem">'+this.publisherCode+'</td>'+
   				'<td class="listItem">'+this.publisherName+'</td>'+
   				'<td class="listItem">'+this.publisherAddress+'</td>'+
   				'<td class="listItem">'+this.publisherPhoneNo+'</td>'+
   			'</tr>';
	});
	return tBody;
}
