		                ...
		                <td nowrap="nowrap"> 	
		              	...
		              	<td nowrap="nowrap"> 	
		                	<b><fmt:message key="label.employeeName"/></b> 
		               		<s:text name="model.employeeName"/>
		              	</td>
	...
	<tr>
    	<td height="100%" valign="top">
			<table cellspacing="0" cellpadding="0" id="tableList" width="100%">
	        	<thead>
	            	<tr>
	            		<td class="hidden"><fmt:message key="label.pkEmployee"/></td>
	            		<td class="listHeader"><fmt:message key="label.employeeCode"/></td>  
	                    <td class="listHeader"><fmt:message key="label.employeeName"/></td>  
	                    <td class="listHeader"><fmt:message key="label.address"/></td>
	                    <td class="listHeader"><fmt:message key="label.phoneNo"/></td>
	                    <td class="listHeader"><fmt:message key="label.department"/></td>
	             	</tr>
	        	</thead>
				<tbody></tbody>	
         	</table>
...
$(document).ready(function(){
	showList(1);
	$('#'+'<%=SystemConstant.MenuCode.EMPLOYEE%>').addClass('tabActive');
});
...
function populateTable(data){
	...
	$.each(data,function(i){
		tBody += 
   			'<tr>'+
   				'<td class="hidden">'+this.pkEmployee+'</td>'+
   				'<td class="listItem">'+this.employeeCode+'</td>'+
   				'<td class="listItem">'+this.employeeName+'</td>'+
   				'<td class="listItem">'+this.address+'</td>'+
   				'<td class="listItem">'+this.phoneNo+'</td>'+
   				'<td class="listItem">'+this.departmentName+'</td>'+
   			'</tr>';
	});
	return tBody;
}
