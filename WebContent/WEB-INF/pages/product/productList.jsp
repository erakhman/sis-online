		    	...
		    	<table cellspacing="0" cellpadding="0" border="0">
					<tr>
		                ...
		                <td nowrap="nowrap"> 	
		              	...
		              	<td nowrap="nowrap"> 	
		                	<b><fmt:message key="label.productName"/></b> 
		               		<s:text name="model.productName"/>
		              	</td>
    	...
    	<td height="100%" valign="top">
			<table cellspacing="0" cellpadding="0" id="tableList" width="100%">
	        	<thead>
	            	<tr>
	            		<td class="hidden"></td>
	            		<td class="listHeader"><fmt:message key="label.productCode"/></td>  
	                    ...
	                    <td class="listHeader"><fmt:message key="label.sellPrice"/></td>
	                    <td class="listHeader"><fmt:message key="label.qty"/></td>  
	                    <td class="listHeader"><fmt:message key="label.merk"/></td>
	                    <td class="listHeader"><fmt:message key="label.madeIn"/></td>    
	             	</tr>
	        	</thead>
				<tbody></tbody>	
         	</table>
...
$(document).ready(function(){
	showList(1);
	$('#'+'<%=SystemConstant.MenuCode.PRODUCT%>').addClass('tabActive');
});
...
function populateTable(data){
	...
	$.each(data,function(i){
		tBody += 
   			'<tr>'+
   				'<td class="hidden">'+this.pkProduct+'</td>'+
   				'<td class="listItem">'+this.productCode+'</td>'+
   				'<td class="listItem">'+this.productName+'</td>'+
   				'<td class="listItem">'+this.categoryName+'</td>'+
   				'<td class="listItem">'+this.unitName+'</td>'+
   				'<td class="listItem">'+formatMoney(this.basicPrice)+'</td>'+
   				'<td class="listItem">'+formatMoney(this.sellPrice)+'</td>'+
   				'<td class="listItem">'+formatMoney(this.qty)+'</td>'+
   				'<td class="listItem">'+this.merk+'</td>'+
   				'<td class="listItem">'+this.madeIn+'</td>'+
   			'</tr>';
	});
	return tBody;
}
